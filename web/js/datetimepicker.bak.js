(function ($) {
    ...
    $.fn.dateTimePicker = function (options) {
        ...
        var timeout = 800;
        var selectDate;
        var defaultValue = elem.next().val();
        if(defaultValue)
            selectDate = moment(defaultValue, settings.dateFormat);
        else {
          selectDate = settings.selectData == "now" ? moment() : moment(settings.selectData, settings.dateFormat);
        ...
        return this.each(function () {
            ...
            function updateMainElemGlobal() {
                if(settings.isDisplayCurrentDate === false){
                  var $s = $('<span>');
                  $s.text(settings.defaultText);
                  elem.empty();
                  elem.append($s);
                }else {
